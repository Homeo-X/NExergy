...
class ForecastBundle:
    ...
    def __post_init__(self) -> None:
        if len(self.pv_points) != self.horizon.n_steps:
            raise ValueError(f"pv_points length {len(self.pv_points)} != horizon.n_steps {self.horizon.n_steps}")
        if len(self.load_points) != self.horizon.n_steps:
            raise ValueError(f"load_points length {len(self.load_points)} != horizon.n_steps {self.horizon.n_steps}")
...
def make_bundle(horizon: TimeHorizon, pv_forecast: PVForecast, load_forecast: LoadForecast,
               reference_temperature_k: float, outdoor_temp_k: float | None = None,
               *, created_at: datetime | None = None) -> ForecastBundle:
    return ForecastBundle(
        bundle_id=str(uuid.uuid4()), created_at=created_at or datetime.now(timezone.utc),
        horizon=horizon, pv_points=tuple(pv_forecast.forecast(horizon)),
        load_points=tuple(load_forecast.forecast(horizon, outdoor_temp_k=outdoor_temp_k)),
