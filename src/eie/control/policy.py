...
Each ControlPolicy maps an optimizer DispatchDecision to one or more
ControlActions targeting specific hardware assets.  Policies enforce
Phase 3 constraints (protected reserve, comfort bounds, reversibility)
at the translation layer — the SafetyGate provides a second independent
check.
"""
...
class ControlPolicy(ABC):
    """Abstract base for control policies."""
    ...
    def actions_from_decision(
        ...
        """Return ControlActions derived from *decision* given the current *snapshot*."""


class BatteryPolicy(ControlPolicy):
    """Battery charge/discharge policy with protected reserve."""
    ...
    def actions_from_decision(
        ...
        if charge_fraction >= 0.5:
            ...
            available_above_reserve = max(
                0.0, (current_soc - self.protected_reserve_fraction) * capacity_j
            )
            ...
            return [
                ControlAction(
                    action_id=str(uuid.uuid4()),
                    timestamp=timestamp,
                    target=ControlTarget.BATTERY_DISCHARGE,
                    target_value=target_w,
                    unit="W",
                    source_decision_id=decision.decision_id,
                    status=ActionStatus.PENDING,
...
class ThermalStoragePolicy(ControlPolicy):
    """Thermal storage charge/discharge within comfort bounds."""
        ...
        if ts_fraction >= 0.5:
            ...
            return [
                ControlAction(
                    action_id=str(uuid.uuid4()),
                    timestamp=timestamp,
                    target=ControlTarget.THERMAL_STORAGE_CHARGE,
                    target_value=target_w,
                    unit="W",
                    source_decision_id=decision.decision_id,
                    status=ActionStatus.PENDING,
                    safety_class="thermal",
                    risk_level="low",
                    is_reversible=True,
                )
            ]
            ...
            return [
                ControlAction(
                    action_id=str(uuid.uuid4()),
                    timestamp=timestamp,
                    target=ControlTarget.THERMAL_STORAGE_DISCHARGE,
                    target_value=target_w,
                    unit="W",
                    source_decision_id=decision.decision_id,
                    status=ActionStatus.PENDING,
...
class PreCoolingPolicy(ControlPolicy):
    """Building pre-cooling setpoint within comfort range."""

    def __init__(
        self,
        comfort_lower_k: float,
        comfort_upper_k: float,
    ...
    def actions_from_decision(
        ...
        return [
            ControlAction(
                action_id=str(uuid.uuid4()),
                timestamp=timestamp,
                target=ControlTarget.PRE_COOL_SETPOINT,
                target_value=setpoint_k,
                unit="K",
                source_decision_id=decision.decision_id,
                status=ActionStatus.PENDING,
                safety_class="thermal",
...
class LoadShiftPolicy(ControlPolicy):
    """Noncritical load deferral policy."""

    def __init__(
        self,
        noncritical_load_ids: list[str],
        max_defer_s: float = 3600.0,
        activation_pv_threshold: float = 0.5,
    ) -> None:
    ...
    def actions_from_decision(
        ...
        actions = []
        for load_id in self.noncritical_load_ids:
            actions.append(
                ControlAction(
                    action_id=str(uuid.uuid4()),
                    timestamp=timestamp,
                    target=ControlTarget.LOAD_DEFER,
                    target_value=self.max_defer_s,
                    unit="s",
                    source_decision_id=decision.decision_id,
                    status=ActionStatus.PENDING,
                    safety_class="load_shift",
                    risk_level="low",
                    is_reversible=True,
                )
            )
...
class WasteHeatPolicy(ControlPolicy):
    """Waste heat routing within safe temperature bounds."""

    def __init__(
        self,
        max_heat_rate_w: float,
        safe_temperature_bounds_k: tuple[float, float],
        activation_hp_threshold: float = 0.6,
    ) -> None:
    ...
    def actions_from_decision(
        ...
        scale = (hp_fraction - self.activation_hp_threshold) / (1.0 - self.activation_hp_threshold)
        target_w = scale * self.max_heat_rate_w
        ...
        return [
            ControlAction(
                action_id=str(uuid.uuid4()),
                timestamp=timestamp,
                target=ControlTarget.WASTE_HEAT_ROUTE,
                target_value=route_temp_k,
                unit="K",
                source_decision_id=decision.decision_id,
                status=ActionStatus.PENDING,
                safety_class="waste_heat",
                risk_level="low",
                is_reversible=False,
            )
        ]
