...
def reference(noon_utc: datetime) -> ReferenceState:
    return ReferenceState(reference_state_id="ref-mpc-test", timestamp=noon_utc,
        ambient_temperature_k=293.15, ambient_pressure_pa=101_325.0, confidence=0.99, valid_until=noon_utc + timedelta(hours=12))

@pytest.fixture
def boundary(reference: ReferenceState, noon_utc: datetime) -> Boundary:
    return Boundary(boundary_id="bnd-mpc-test", boundary_type=BoundaryType.SITE,
        included_entity_ids=["battery", "ts"], excluded_entity_ids=[],
        reference_state_id=reference.reference_state_id,
        accounting_period_start=noon_utc, accounting_period_end=noon_utc + timedelta(hours=12))

@pytest.fixture
def meta(reference: ReferenceState, boundary: Boundary, noon_utc: datetime) -> Metadata:
    return Metadata(timestamp=noon_utc, source="mpc-test", method=MeasurementMethod.SIMULATED, unit="J",
        confidence=1.0, uncertainty=0.0, boundary_id=boundary.boundary_id, reference_state_id=reference.reference_state_id)

@pytest.fixture
def snapshot(reference: ReferenceState, boundary: Boundary, meta: Metadata, noon_utc: datetime) -> SiteSnapshot:
    ...
    return SiteSnapshot(snapshot_id="snap-mpc-001", timestamp=noon_utc, reference_state=reference, boundary=boundary,
...
def bundle(horizon_4h: TimeHorizon) -> "ForecastBundle":  # type: ignore[name-defined]
    ...
    return make_bundle(horizon_4h, PVForecast(panel_area_m2=20.0, panel_efficiency=0.20),
        LoadForecast(base_thermal_w=5000.0, base_electrical_w=2000.0), reference_temperature_k=293.15)

@pytest.fixture
def scheduler() -> MPCScheduler:
    return MPCScheduler(optimizer=ShadowOptimizer(grid_points_per_variable=5),
        policies=[BatteryPolicy(protected_reserve_fraction=0.20), ThermalStoragePolicy(min_comfort_k=293.15, max_comfort_k=340.0)],
        gate=SafetyGate())

def test_schedule_step_count(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    schedule = scheduler.schedule(snapshot, bundle, created_at=noon_utc)
    assert isinstance(schedule, MPCSchedule) and len(schedule.steps) == bundle.horizon.n_steps

def test_schedule_snapshot_id(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    assert scheduler.schedule(snapshot, bundle, created_at=noon_utc).snapshot_id == snapshot.snapshot_id

def test_schedule_total_improvement_non_negative(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    assert scheduler.schedule(snapshot, bundle, created_at=noon_utc).total_forecast_exergy_improvement_j >= 0.0

def test_schedule_is_frozen(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    with pytest.raises((AttributeError, TypeError)):
        scheduler.schedule(snapshot, bundle, created_at=noon_utc).snapshot_id = "tampered"  # type: ignore[misc]

def test_scheduled_action_per_step(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    for step_action in scheduler.schedule(snapshot, bundle, created_at=noon_utc).steps:
        assert isinstance(step_action, ScheduledAction)
        assert len(step_action.gate_verdicts) == len(step_action.control_actions)

def test_verdicts_match_actions(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    for sa in scheduler.schedule(snapshot, bundle, created_at=noon_utc).steps:
        assert {a.action_id for a in sa.control_actions} == {v.action_id for v in sa.gate_verdicts}

def test_approved_actions_have_approved_verdicts(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    for sa in scheduler.schedule(snapshot, bundle, created_at=noon_utc).steps:
...
def test_first_step_actions_only_approved(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    schedule = scheduler.schedule(snapshot, bundle, created_at=noon_utc)
...
def test_approved_steps_property(scheduler: MPCScheduler, snapshot: SiteSnapshot, bundle: "ForecastBundle", noon_utc: datetime) -> None:  # type: ignore[name-defined]
    assert all(s.approved for s in scheduler.schedule(snapshot, bundle, created_at=noon_utc).approved_steps)
...
def test_schedule_length_mismatch(horizon_4h: TimeHorizon, noon_utc: datetime) -> None:
    ...
    dummy_step = ForecastStep(step_index=0, start=noon_utc, end=noon_utc + timedelta(hours=1), duration_s=3600.0)
    opt = OptimizationResult(result_id="r", timestamp=noon_utc, snapshot_id="s",
        feasible_decisions=[], pareto_front=[], recommended_decision=None,
        objective_scores={}, evaluated_count=0, infeasible_count=0, guard_bypass_count=0, objectives=list(STANDARD_OBJECTIVES))
    sa = ScheduledAction(step=dummy_step, optimization_result=opt, control_actions=[], gate_verdicts=[], approved=False, forecast_exergy_improvement_j=0.0)
    with pytest.raises(ValueError, match="n_steps"):
        MPCSchedule(schedule_id="s1", created_at=noon_utc, horizon=horizon_4h, snapshot_id="snap",
            steps=[sa], total_forecast_exergy_improvement_j=0.0)
