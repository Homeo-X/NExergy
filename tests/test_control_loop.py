...
def reference(now: datetime) -> ReferenceState:
    return ReferenceState(
        reference_state_id="ref-loop-test",
        timestamp=now,
...
def boundary(reference: ReferenceState, now: datetime) -> Boundary:
    return Boundary(
        ...
        accounting_period_end=now + timedelta(hours=2),
    )
...
def meta(reference: ReferenceState, boundary: Boundary, now: datetime) -> Metadata:
    return Metadata(
        timestamp=now, source="loop-test", method=MeasurementMethod.SIMULATED,
...
def snapshot(reference: ReferenceState, boundary: Boundary, meta: Metadata, now: datetime) -> SiteSnapshot:
    ...
    return SiteSnapshot(
        snapshot_id="snap-loop-001", timestamp=now,
        reference_state=reference, boundary=boundary,
        horizon_duration_s=3600.0, available_pv_power_w=6_000.0,
...
def controller(snapshot: SiteSnapshot) -> ClosedLoopController:
    ...
    gate = SafetyGate()
    config = LoopConfig(optimizer=optimizer, policies=policies, gate=gate)
    return ClosedLoopController(config)


def test_step_returns_loop_iteration(controller: ClosedLoopController,
                                      snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    assert isinstance(iteration, LoopIteration)


def test_guard_bypass_count_always_zero(controller: ClosedLoopController,
                                         snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    assert iteration.guard_bypass_count == 0


def test_approved_actions_are_gate_verified(controller: ClosedLoopController,
                                             snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    ...
    for verdict in iteration.gate_verdicts:
        if verdict.action_id in approved_ids:
            assert verdict.approved, f"Approved action {verdict.action_id} has rejected verdict"


def test_rejected_actions_not_approved(controller: ClosedLoopController,
                                        snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
...
def test_exergy_improvement_non_negative(controller: ClosedLoopController,
                                          snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    assert iteration.exergy_improvement_j >= 0.0


def test_snapshot_id_recorded(controller: ClosedLoopController,
                               snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    assert iteration.snapshot_id == snapshot.snapshot_id


def test_iteration_id_unique(controller: ClosedLoopController,
                              snapshot: SiteSnapshot, now: datetime) -> None:
    it1 = controller.step(snapshot, at=now)
    it2 = controller.step(snapshot, at=now)
    assert it1.iteration_id != it2.iteration_id


def test_loop_iteration_frozen(controller: ClosedLoopController,
                                snapshot: SiteSnapshot, now: datetime) -> None:
    iteration = controller.step(snapshot, at=now)
    with pytest.raises((AttributeError, TypeError)):
        iteration.guard_bypass_count = 1  # type: ignore[misc]


def test_loop_iteration_bypass_invariant() -> None:
    with pytest.raises(ValueError, match="guard_bypass_count"):
        LoopIteration(
            iteration_id="x", timestamp=datetime.now(timezone.utc),
            snapshot_id="s", optimization_result=None,  # type: ignore[arg-type]
            proposed_actions=[], approved_actions=[], rejected_actions=[],
            execution_results=[], gate_verdicts=[], exergy_improvement_j=0.0,
            guard_bypass_count=1,
        )


def test_run_returns_one_iteration_per_snapshot(controller: ClosedLoopController,
                                                  snapshot: SiteSnapshot, now: datetime) -> None:
    snapshots = [snapshot, snapshot, snapshot]
    iterations = controller.run(snapshots, at=now)
    assert len(iterations) == 3
    assert all(isinstance(it, LoopIteration) for it in iterations)


def test_run_all_guard_bypass_zero(controller: ClosedLoopController,
                                    snapshot: SiteSnapshot, now: datetime) -> None:
    iterations = controller.run([snapshot, snapshot], at=now)
    for it in iterations:
        assert it.guard_bypass_count == 0


def test_action_log_grows_with_iterations(controller: ClosedLoopController,
                                           snapshot: SiteSnapshot, now: datetime) -> None:
...
def test_step_with_no_feasible_decisions(boundary: Boundary, reference: ReferenceState,
                                          meta: Metadata, now: datetime) -> None:
    optimizer = ShadowOptimizer(grid_points_per_variable=2)
    gate = SafetyGate()
    config = LoopConfig(optimizer=optimizer, policies=[BatteryPolicy()], gate=gate)
    ctrl = ClosedLoopController(config)

    battery_state = BatteryState(
        storage_id="b", stored_energy_j=1_000_000.0, soc=0.5, soh=0.9,
        reserve_energy_j=100_000.0, boundary_id=boundary.boundary_id,
        reference_state_id=reference.reference_state_id, metadata=meta,
    )
    ts_state = ThermalStorageState(
        storage_id="ts", layers=[ThermalLayer(temperature_k=320.0, energy_j=1_000_000.0)],
        boundary_id=boundary.boundary_id, reference_state_id=reference.reference_state_id,
        metadata=meta,
    )
    snap = SiteSnapshot(
        snapshot_id="minimal-snap", timestamp=now, reference_state=reference, boundary=boundary,
        horizon_duration_s=3600.0, available_pv_power_w=1_000.0,
        battery_state=battery_state,
        battery_constraints=BatteryConstraints(
            max_charge_power_w=1000.0, max_discharge_power_w=1000.0,
            min_soc=0.10, max_soc=0.95, round_trip_efficiency=0.9,
            capacity_j=3_600_000.0 * 5,
        ),
        thermal_storage_state=ts_state,
        thermal_storage_constraints=ThermalStorageConstraints(
            max_charge_power_w=1000.0, max_discharge_power_w=1000.0,
        ...
        building_electric_load_w=800.0, heat_pump_rated_cop=3.0,
        heat_pump_max_power_w=1000.0, delivered_heat_temperature_k=313.15,
        heat_demand_w=2000.0,
        grid_constraints=GridConstraints(
