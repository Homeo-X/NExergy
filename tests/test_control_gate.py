...
def reference(now: datetime) -> ReferenceState:
    return ReferenceState(
        reference_state_id="ref-gate-test",
        timestamp=now,
...
def _action(action_id: str, target: ControlTarget, value: float, unit: str,
             safety_class: str, risk_level: str = "low",
             is_reversible: bool = True, now: datetime | None = None) -> ControlAction:
...
def test_allowed_classes_approved(gate: SafetyGate, snapshot: SiteSnapshot,
                                   safety_class: str, target: ControlTarget,
                                   value: float, unit: str) -> None:
    act = _action("a1", target, value, unit, safety_class)
    verdict = gate.evaluate(act, snapshot)
    assert verdict.approved, f"Expected approval for {safety_class}: {verdict.reasons}"


@pytest.mark.parametrize("blocked_class", [
    "critical_load", "hydrogen", "emergency_reserve", "grid_islanding", "fleet_trade",
])
def test_blocked_classes_rejected(gate: SafetyGate, snapshot: SiteSnapshot,
                                   blocked_class: str) -> None:
    act = _action("b1", ControlTarget.BATTERY_CHARGE, 100.0, "W", blocked_class)
    verdict = gate.evaluate(act, snapshot)
...
def test_unknown_class_rejected(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("u1", ControlTarget.BATTERY_CHARGE, 100.0, "W", "unknown_class")
    verdict = gate.evaluate(act, snapshot)
    assert not verdict.approved


def test_medium_risk_rejected(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("m1", ControlTarget.BATTERY_CHARGE, 100.0, "W", "battery", risk_level="medium")
    verdict = gate.evaluate(act, snapshot)
    assert not verdict.approved


def test_battery_discharge_below_reserve_rejected(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("d1", ControlTarget.BATTERY_DISCHARGE, 9000.0, "W", "battery")
    verdict = gate.evaluate(act, snapshot)
    assert not verdict.approved
...
def test_battery_discharge_above_reserve_approved(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("d2", ControlTarget.BATTERY_DISCHARGE, 200.0, "W", "battery")
    verdict = gate.evaluate(act, snapshot)
    assert verdict.approved


def test_battery_charge_exceeds_max_rejected(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("c1", ControlTarget.BATTERY_CHARGE, 5000.0, "W", "battery")
    verdict = gate.evaluate(act, snapshot)
    assert not verdict.approved


def test_thermal_setpoint_below_comfort_rejected(gate: SafetyGate, snapshot: SiteSnapshot) -> None:
    act = _action("t1", ControlTarget.BUILDING_SETPOINT, 285.0, "K", "thermal")
    verdict = gate.evaluate(act, snapshot)
    assert not verdict.approved
...
def test_custom_gate_tighter_reserve(snapshot: SiteSnapshot) -> None:
    ...
    act = _action("cr1", ControlTarget.BATTERY_DISCHARGE, 200.0, "W", "battery")
    verdict = custom_gate.evaluate(act, snapshot)
    assert not verdict.approved
