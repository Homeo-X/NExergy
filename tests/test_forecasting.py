...
def test_horizon_end(horizon_24h: TimeHorizon, noon_utc: datetime) -> None:
    expected_end = noon_utc + timedelta(hours=24)
    assert horizon_24h.end == expected_end
...
def test_horizon_step_at(horizon_24h: TimeHorizon) -> None:
    step = horizon_24h.step_at(0)
    assert step.step_index == 0
    step_last = horizon_24h.step_at(23)
    assert step_last.step_index == 23


def test_horizon_step_at_out_of_range(horizon_24h: TimeHorizon) -> None:
    with pytest.raises(IndexError):
        horizon_24h.step_at(24)
    with pytest.raises(IndexError):
        horizon_24h.step_at(-1)
...
def test_horizon_invalid_step_duration() -> None:
    with pytest.raises(ValueError):
        TimeHorizon(start=datetime(2026, 1, 1, tzinfo=timezone.utc),
                    step_duration_s=0.0, n_steps=10)


def test_horizon_invalid_n_steps() -> None:
    with pytest.raises(ValueError):
        TimeHorizon(start=datetime(2026, 1, 1, tzinfo=timezone.utc),
                    step_duration_s=3600.0, n_steps=0)


def test_forecast_step_midpoint(horizon_short: TimeHorizon) -> None:
    step = horizon_short.steps[0]
    expected_mid = step.start + timedelta(seconds=450.0)
    assert step.midpoint == expected_mid
...
def test_pv_noon_positive(pv_forecast: PVForecast, noon_utc: datetime) -> None:
    ...
    points = pv_forecast.forecast(horizon)
    assert points[0].pv_power_w > 0
    assert points[0].irradiance_w_m2 > 0


def test_pv_midnight_zero(pv_forecast: PVForecast, midnight_utc: datetime) -> None:
    horizon = TimeHorizon(start=midnight_utc, step_duration_s=3600.0, n_steps=1)
    points = pv_forecast.forecast(horizon)
    assert points[0].pv_power_w == pytest.approx(0.0)
    assert points[0].irradiance_w_m2 == pytest.approx(0.0)


def test_pv_cloud_degradation(noon_utc: datetime) -> None:
    clear = PVForecast(panel_area_m2=10.0, panel_efficiency=0.20, cloud_degradation_factor=1.0)
    cloudy = PVForecast(panel_area_m2=10.0, panel_efficiency=0.20, cloud_degradation_factor=0.4)
    horizon = TimeHorizon(start=noon_utc, step_duration_s=3600.0, n_steps=1)
    clear_pts = clear.forecast(horizon)
    cloudy_pts = cloudy.forecast(horizon)
    assert cloudy_pts[0].pv_power_w < clear_pts[0].pv_power_w
...
def test_pv_forecast_invalid_efficiency() -> None:
    with pytest.raises(ValueError):
        PVForecast(panel_area_m2=10.0, panel_efficiency=1.5)


def test_pv_forecast_invalid_area() -> None:
    with pytest.raises(ValueError):
        PVForecast(panel_area_m2=-1.0, panel_efficiency=0.2)


def test_pv_forecast_invalid_cloud() -> None:
    with pytest.raises(ValueError):
        PVForecast(panel_area_m2=10.0, panel_efficiency=0.2, cloud_degradation_factor=1.5)
...
def test_load_forecast_length(load_forecast: LoadForecast, horizon_24h: TimeHorizon) -> None:
    points = load_forecast.forecast(horizon_24h)
    assert len(points) == 24
...
def test_load_forecast_non_negative(load_forecast: LoadForecast, horizon_24h: TimeHorizon) -> None:
    points = load_forecast.forecast(horizon_24h)
    for p in points:
        assert p.thermal_load_w >= 0.0
        assert p.electrical_base_load_w >= 0.0
        assert p.noncritical_electrical_w >= 0.0


def test_load_noncritical_fraction(load_forecast: LoadForecast, noon_utc: datetime) -> None:
    horizon = TimeHorizon(start=noon_utc, step_duration_s=3600.0, n_steps=1)
    points = load_forecast.forecast(horizon)
    ratio = points[0].noncritical_electrical_w / points[0].electrical_base_load_w
    assert ratio == pytest.approx(0.3, abs=1e-6)
...
def test_load_temperature_correction(horizon_24h: TimeHorizon) -> None:
    lf = LoadForecast(base_thermal_w=5000.0, base_electrical_w=2000.0,
                      temperature_sensitivity_w_per_k=100.0,
                      reference_outdoor_temp_k=283.15)
...
def test_load_forecast_invalid_profile() -> None:
    with pytest.raises(ValueError):
        LoadForecast(base_thermal_w=1000.0, base_electrical_w=1000.0,
                     hourly_thermal_profile=[1.0] * 12)


def test_load_forecast_invalid_noncritical() -> None:
    with pytest.raises(ValueError):
        LoadForecast(base_thermal_w=1000.0, base_electrical_w=1000.0,
                     noncritical_fraction=1.5)


@pytest.fixture
def bundle(horizon_24h: TimeHorizon, pv_forecast: PVForecast,
           load_forecast: LoadForecast) -> ForecastBundle:
    return make_bundle(horizon_24h, pv_forecast, load_forecast,
                       reference_temperature_k=293.15)


def test_bundle_step_count(bundle: ForecastBundle, horizon_24h: TimeHorizon) -> None:
    assert len(bundle.pv_points) == horizon_24h.n_steps
    assert len(bundle.load_points) == horizon_24h.n_steps


def test_bundle_pv_at(bundle: ForecastBundle) -> None:
    point = bundle.pv_at(0)
    assert isinstance(point, PVForecastPoint)


def test_bundle_load_at(bundle: ForecastBundle) -> None:
    point = bundle.load_at(0)
...
def test_bundle_length_mismatch(horizon_24h: TimeHorizon) -> None:
    ...
    pv_pts = tuple(pv.forecast(horizon_24h))
    load_pts = tuple(lf.forecast(horizon_24h))
